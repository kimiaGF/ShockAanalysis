...
if __name__ == '__main__':
    fs =glob(f'/home/kimia.gh/blue2/B4C_ML_Potential/B4C_LAMMPS/DPMD_GPU/orientation_shock_50-50-1000/B12-CBC/0/mid_chain/*/shock_-25.0_data/dump.data_shock_chunk_*') 
    
    fs_dirs = [f.split('dump.data_shock_chunk')[0] for f in fs]
    files = fs
    # files = []
    # for d in fs_dirs:
    #     if os.path.isfile(os.path.join(d,'processed_data.pkl')):
    #         files.append(glob(os.path.join(d,'dump.data_shock_chunk_*'))[0])
    
    #extract ts from log file
    # velocity,tstep = extract_meta_from_log(log_file[0])

    sim_df = process_files(files,ts=1.0)

    all_data = get_large_dataframe(sim_df)

#%% visualize the data
all_data.drop(columns=['vac_frac','removal'],inplace=True)
all_data.set_index(['polytype','angle','velocity'],inplace=True)
all_data
...
polytype = 'B12-CBC'
velocity = -25.0
subset = all_data.loc[idx[polytype,angle,velocity],:]
...
fig.update_layout(academic_template['layout'])
# removes the transition between frames and speeds up animation
...
#%%
#find orientation with highest stress for each bin 
timesteps = subset['timestep'].unique()
#only looking at one timestep
# sigma_z = []
# max_stress = []
# sigma_z_prime = []
# for t in timesteps:
#     data = subset.loc[subset['timestep']== t]
#     data = data.loc[data['bin_id']==30]

#     #for each bin, get the stress tensor and find its eigenvectors
#     sigma = np.array(
#         [[data['sig x'],data['txy'],data['txz']],
#         [data['txy'],data['sig y'],data['tyz']],
#         [data['txz'],data['tyz'],data['sig z']]]
#         ).reshape(3,3)

#     principal_stresses, principal_directions = np.linalg.eig(sigma)
...
#     sigma_prime = np.dot(R, np.dot(sigma, R.T))
#     max_stress.append(max(principal_stresses))
#     sigma_z.append(sigma[-1,-1])
#     sigma_z_prime.append(sigma_prime[-1,-1])

#%%
#find the plastic wave velocity for each angle 
#use a timestep for development purposes
timesteps = subset['timestep'].unique()
angles = subset['angle'].unique()
angle_speed_df = pd.DataFrame(columns=['angle','p_pos','e_pos','p_time','e_time'])
angle_speed_df['angle'] = angles
angle_speed_df['p_pos'] = np.nan
angle_speed_df['e_pos'] = np.nan
angle_speed_df['p_time'] = np.nan
angle_speed_df['e_time'] = np.nan

angle_speed_df.set_index('angle',inplace=True)

for angle in angles:
    sub_data = subset.loc[subset['angle'] == angle]
    pressure = gaussian_filter1d(sub_data['Hydrostatic Pressure'].values,1)
    sub_data['pressure_sm'] =  pressure
    z = sub_data['z'].values

    data = sub_data.copy()
    smoothed_data = pd.DataFrame()
    df = {'z':[],'press_diff':[],'timestep':[]}
    for t in timesteps:
        sub_data = data.loc[data['timestep'] == t]
        pressure = gaussian_filter1d(sub_data['Hydrostatic Pressure'].values,1)
        df['z'] = sub_data['z'][1:]
        df['bin_id'] = sub_data['bin_id'][1:]
        df['pressure_sm_g'] = pressure[1:]
        df['press_diff'] = np.diff(pressure)
        df['timestep'] = t
        
        smoothed_data = pd.concat([smoothed_data,pd.DataFrame(df)],ignore_index=True)
    
    timesteps = smoothed_data.timestep.unique()

    #get middle 50% of timestep
    time_subset = list(timesteps[int(len(timesteps)/10):int(len(timesteps)*9/10)])

    wave_data = {'e_timestep':[],'p_timestep':[],'elastic_front':[],'plastic_front':[]}
    for timestep in time_subset:
        subset_sm = smoothed_data.loc[smoothed_data['timestep'] == timestep]

        #find the two peaks in press_diff
        lowest_values = subset_sm.loc[subset_sm['z'].between(30,970)].nsmallest(20,'press_diff')
        
        #filter out values above 50% of minimum
        lowest_values = lowest_values.loc[lowest_values['press_diff'] < 0.25*lowest_values['press_diff'].min()]
        
        # average z values that are very close to each other within a threshold
        lowest_values['z_bin'] = pd.cut(lowest_values['z'], bins=2,labels=False)
        

        #find the z with the lowest press_diff within each zbin
        lowest_values = lowest_values.loc[lowest_values.groupby('z_bin')['press_diff'].idxmin()]

        try:
            elastic_front = lowest_values.loc[lowest_values['z'] == lowest_values['z'].max()]['z'].values[0]
            # print(elastic_front)
            # print(wave_data['elastic_front'])
            
            wave_data['e_timestep'].append(timestep)
            wave_data['elastic_front'].append(elastic_front)
            # print(elastic_front)

            # print(wave_data['elastic_front'][-1])
                
        except Exception as e:
            print(e)
            elastic_front = 0
            
        # assign plastic front location based on lowest z value
        try:
            plastic_front = lowest_values.loc[lowest_values['z'] == lowest_values['z'].min()]['z'].values[0]
        
            wave_data['p_timestep'].append(timestep)
            wave_data['plastic_front'].append(plastic_front)
        except:
            plastic_front = 0
    # #remove outliers
    # diffs = np.diff(np.diff(wave_data['elastic_front']))
    # outliers = np.where(abs(diffs) >= 20)
    # # outliers = outliers[0] + 2
    # p_diffs = np.diff(np.diff(wave_data['plastic_front']))
    # p_outliers = np.where(abs(p_diffs) >= 20)

    # wave_data['elastic_front'] = np.delete(wave_data['elastic_front'],outliers[0]+2)
    # wave_data['e_timestep'] = np.delete(wave_data['e_timestep'],outliers[0]+2)
    # wave_data['plastic_front'] = np.delete(wave_data['plastic_front'],p_outliers[0]+2)
    # wave_data['p_timestep'] = np.delete(wave_data['p_timestep'],p_outliers[0]+2)

    
    
    #find the slope of the elastic front
    e_slope, e_intercept, r_value, p_value, std_err = linregress(wave_data['e_timestep'],wave_data['elastic_front'])
    print(f'Elastic front slope: {e_slope}')

    #find the slope of the plastic front
    p_slope, p_intercept, r_value, p_value, std_err = linregress(wave_data['p_timestep'],wave_data['plastic_front'])
    print(f'Plastic front slope: {p_slope}')

    plt.rcParams['font.family'] = 'DejaVu Serif'
    # px.scatter(wave_data['e_timestep'], wave_data['elastic_front'], label=f'Elastic front: {e_slope*100:.2f} km/s')
    # px.scatter(wave_data['p_timestep'], wave_data['plastic_front'], label=f'Plastic front: {p_slope*100:.2f} km/s')

    fig = px.scatter(
        wave_data,
        x='e_timestep',
        y='elastic_front',
        title=f'{polytype} at {angle}º elastic'
    )
    ...
    fig = px.scatter(
        ...
        x='p_timestep',
        y='plastic_front',
        title=f'{polytype} at {angle}º plastic'
    )
    ...
    # Plot the line of best fit for elastic front
    e_line = [e_slope * x + e_intercept for x in wave_data['e_timestep']]
    # px.plot(wave_data['e_timestep'], e_line, color='blue')
    # px.line(wave_data['e_timestep'], e_line)

    # Plot the line of best fit for plastic front
    p_line = [p_slope * x + p_intercept for x in wave_data['p_timestep']]
    # px.plot(wave_data['p_timestep'], p_line, color='orange')
    # px.line(wave_data['p_timestep'], p_line)

    # plt.title(f'{polytype} at {angle}º')
    # plt.xlabel('Timestep (fs)', fontsize=14)
    # plt.ylabel('Z ($\AA$)', fontsize=14)
    # plt.legend()
...
# fig.show()    
#
# #------------------------------ Plot the hydrostatic pressure profile over time ------------------------------
# fig = px.line(
#     smoothed_data,
#     x='z',
#     y='pressure_sm_g',
...
#%%
# track the location of the shock front over time
#use a timestep for development purposes


# %%
#find slopes for plastic and elastic fronts 
...
for v_imp in all_data_noidx.velocity.unique():
    subset = all_data.loc[idx[:,:,:,0.0,v_imp],:]
